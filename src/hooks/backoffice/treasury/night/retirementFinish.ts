import { fetchAllRetirementFinish } from "@/api/endpoints/treasury/night/retirementFinish";
import { QueryTables } from "@/types";
import { CalendarDate } from "@nextui-org/react";
import { useState } from "react";
import useSWR from "swr";

export function useAllRetirementFinish(date: {
  start: CalendarDate;
  end: CalendarDate;
}) {
  ...
  const { data, isLoading, mutate } = useSWR(
    `/backoffice/treasury/night/retirementFinish?page=${queries.page}&limit=${
      queries.limit
    }&q=${queries.q}${
    ...
    (url: string) => fetchAllRetirementFinish(url),
    {
