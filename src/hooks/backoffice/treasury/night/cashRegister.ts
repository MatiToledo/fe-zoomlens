import { fetchAllCashRegister } from "@/api/endpoints/cashRegister";
import { QueryTables } from "@/types";
import { CalendarDate } from "@nextui-org/react";
import { useState } from "react";
import useSWR from "swr";

export function useAllCashRegister(date: {
  start: CalendarDate;
  end: CalendarDate;
}) {
  ...
  const { data, isLoading, mutate } = useSWR(
    `/backoffice/cashRegister?page=${queries.page}&limit=${queries.limit}&q=${
      queries.q
    ...
    (url: string) => fetchAllCashRegister(url),
    {
