import { fetchAllPostnetMP } from "@/api/endpoints/treasury/central/postnet/mp";
import { QueryTables } from "@/types";
import { CalendarDate } from "@nextui-org/react";
import { useState } from "react";
import useSWR from "swr";

export function useAllPostnetMP(date: {
  start: CalendarDate;
  end: CalendarDate;
}) {
  ...
  const { data, isLoading, mutate } = useSWR(
    `/backoffice/treasury/central/postnet/mp?page=${queries.page}&limit=${
      queries.limit
    }&q=${queries.q}${
    ...
    (url: string) => fetchAllPostnetMP(url),
    {
