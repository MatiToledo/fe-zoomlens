import { fetchAllRegisterUSD } from "@/api/endpoints/treasury/central/register/usd";
import { QueryTables } from "@/types";
import { CalendarDate } from "@nextui-org/react";
import { useState } from "react";
import useSWR from "swr";

export function useAllRegisterUSD(date: {
  start: CalendarDate;
  end: CalendarDate;
}) {
  ...
  const { data, isLoading, mutate } = useSWR(
    `/backoffice/treasury/central/register/usd?page=${queries.page}&limit=${
      queries.limit
    }&q=${queries.q}${
    ...
    (url: string) => fetchAllRegisterUSD(url),
    {
