import { fetchAllRegisterBank } from "@/api/endpoints/treasury/central/register/bank";
import { QueryTables } from "@/types";
import { CalendarDate } from "@nextui-org/react";
import { useState } from "react";
import useSWR from "swr";

export function useAllRegisterBank(date: {
  start: CalendarDate;
  end: CalendarDate;
}) {
  ...
  const { data, isLoading, mutate } = useSWR(
    `/backoffice/treasury/central/register/bank?page=${queries.page}&limit=${
      queries.limit
    }&q=${queries.q}${
    ...
    (url: string) => fetchAllRegisterBank(url),
    {
