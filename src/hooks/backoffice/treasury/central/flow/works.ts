import { fetchAllFlowWorks } from "@/api/endpoints/treasury/central/flow/works";
import { QueryTables } from "@/types";
import { CalendarDate } from "@nextui-org/react";
import { useState } from "react";
import useSWR from "swr";

export function useAllFlowWorks(date: {
  start: CalendarDate;
  end: CalendarDate;
}) {
  ...
  const { data, isLoading, mutate } = useSWR(
    `/backoffice/treasury/central/flow/works?page=${queries.page}&limit=${
      queries.limit
    }&q=${queries.q}${
    ...
    (url: string) => fetchAllFlowWorks(url),
    {
