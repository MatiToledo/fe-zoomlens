import {
  fetchCreateRegisterTicket,
  fetchDeleteRegisterTicket,
  fetchEditRegisterTicket,
} from "@/api/endpoints/register/ticket";
...
export default function RegisterTicketForm({
  ...
  async function onSubmit(data: any) {
    ...
    isEdit
      ? await fetchEditRegisterTicket(rowSelected.id, data)
      : await fetchCreateRegisterTicket(data);
    mutate();
    reset();
    setRowSelected(null);
    onClose();
    notify(
      `Boleteria ${isEdit ? "actualizada" : "creada"} con exito`,
      "success"
    );
  }

  async function handleDelete() {
    await fetchDeleteRegisterTicket(rowSelected.id);
    mutate();
    reset();
    setRowSelected(null);
    onClose();
    notify("Boleteria eliminada con exito", "success");
  }
    ...
    <Modal
      ...
      <ModalContent>
        <ModalHeader className="flex flex-col gap-1">
          {isEdit ? "Editar boleteria" : "Crear boleteria"}
        </ModalHeader>
