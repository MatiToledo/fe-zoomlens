...
import { useAllPostnetMP } from "@/hooks/backoffice/treasury/central/postnet/mp";
...
export default function TableTreasuryCentralPostnetMP() {
  const [rowSelected, setRowSelected] = useState<PostnetMP | null>(null);
  const [cookies, setCookie] = useCookies(["user"]);
  const [date, setDate] = useState({
    start: parseDate(getInitialsDate("start")),
    end: parseDate(getInitialsDate("end")),
  });
  const { rows, count, setQueries, queries, mutate } = useAllPostnetMP(date);
