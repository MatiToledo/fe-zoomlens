...
import { useAllRegisterMP } from "@/hooks/backoffice/treasury/central/register/mp";
...
export default function TableTreasuryCentralRegisterMP() {
  const [rowSelected, setRowSelected] = useState<RegisterMP | null>(null);
  const [cookies, setCookie] = useCookies(["user"]);
  const [date, setDate] = useState({
    start: parseDate(getInitialsDate("start")),
    end: parseDate(getInitialsDate("end")),
  });
  const { rows, count, setQueries, queries, mutate } = useAllRegisterMP(date);
