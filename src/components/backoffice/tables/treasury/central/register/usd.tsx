...
import { useAllRegisterUSD } from "@/hooks/backoffice/treasury/central/register/usd";
...
export default function TableTreasuryCentralRegisterUSD() {
  const [rowSelected, setRowSelected] = useState<RegisterUSD | null>(null);
  const [cookies, setCookie] = useCookies(["user"]);
  const [date, setDate] = useState({
    start: parseDate(getInitialsDate("start")),
    end: parseDate(getInitialsDate("end")),
  });
  const { rows, count, setQueries, queries, mutate } = useAllRegisterUSD(date);
