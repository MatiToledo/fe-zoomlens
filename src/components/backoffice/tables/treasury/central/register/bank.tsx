...
import { useAllRegisterBank } from "@/hooks/backoffice/treasury/central/register/bank";
...
export default function TableTreasuryCentralRegisterBank() {
  const [rowSelected, setRowSelected] = useState<RegisterBank | null>(null);
  const [cookies, setCookie] = useCookies(["user"]);
  const [date, setDate] = useState({
    start: parseDate(getInitialsDate("start")),
    end: parseDate(getInitialsDate("end")),
  });
  const { rows, count, setQueries, queries, mutate } = useAllRegisterBank(date);
