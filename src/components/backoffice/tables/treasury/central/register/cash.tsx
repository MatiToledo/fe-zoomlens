...
import { useAllRegisterCash } from "@/hooks/backoffice/treasury/central/register/cash";
...
export default function TableTreasuryCentralRegisterCash() {
  const [rowSelected, setRowSelected] = useState<RegisterCash | null>(null);
  const [cookies, setCookie] = useCookies(["user"]);
  const [date, setDate] = useState({
    start: parseDate(getInitialsDate("start")),
    end: parseDate(getInitialsDate("end")),
  });
  const { rows, count, setQueries, queries, mutate } = useAllRegisterCash(date);
