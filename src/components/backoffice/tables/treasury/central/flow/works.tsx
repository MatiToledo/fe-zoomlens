...
import { useAllFlowWorks } from "@/hooks/backoffice/treasury/central/flow/works";
...
export default function TableTreasuryCentralFlowWorks() {
  const [rowSelected, setRowSelected] = useState<FlowWorks | null>(null);
  const [cookies, setCookie] = useCookies(["user"]);
  const [date, setDate] = useState({
    start: parseDate(getInitialsDate("start")),
    end: parseDate(getInitialsDate("end")),
  });
  const { rows, count, setQueries, queries, mutate } = useAllFlowWorks(date);
