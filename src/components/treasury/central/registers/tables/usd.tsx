...
import { fetchBulkCreateRegisterUSD } from "@/api/endpoints/treasury/central/register/usd";
import { useGetConceptsByLevel } from "@/hooks/concept";
import { useAllTreasuryCentralRegisterUSD } from "@/hooks/treasury/central/register/usd";
import { selectColumn } from "@/ui/grid/select";
import GridComponent from "../../grid";

export default function TableTreasuryCentralRegisterUSD({
  user,
  title,
}: {
  user: User;
  ...
  const { movements, setMovements, mutate } = useAllTreasuryCentralRegisterUSD(
    user.BranchId,
    date
  );
    ...
    <div className="bg-default-300 grow overflow-x-hidden w-full p-5">
      ...
      {concepts.length > 0 && movements && (
        <GridComponent
          ...
          mutate={mutate}
          BranchId={user.BranchId}
          fetcher={fetchBulkCreateRegisterUSD}
        />
      )}
    </div>
  );
