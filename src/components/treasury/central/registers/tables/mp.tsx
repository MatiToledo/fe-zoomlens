...
import { fetchBulkCreateRegisterMP } from "@/api/endpoints/treasury/central/register/mp";
import { useGetConceptsByLevel } from "@/hooks/concept";
import { useAllTreasuryCentralRegisterMP } from "@/hooks/treasury/central/register/mp";
import { selectColumn } from "@/ui/grid/select";
import GridComponent from "../../grid";

export default function TableTreasuryCentralRegisterMP({
  user,
  title,
}: {
  user: User;
  ...
  const { movements, setMovements, mutate } = useAllTreasuryCentralRegisterMP(
    user.BranchId,
    date
  );
    ...
    <div className="bg-default-300 grow overflow-x-hidden w-full p-5">
      ...
      {concepts.length > 0 && movements && (
        <GridComponent
          ...
          mutate={mutate}
          BranchId={user.BranchId}
          fetcher={fetchBulkCreateRegisterMP}
        />
      )}
    </div>
  );
