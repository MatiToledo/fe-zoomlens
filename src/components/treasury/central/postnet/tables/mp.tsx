...
import { fetchBulkCreatePostnetMP } from "@/api/endpoints/treasury/central/postnet/mp";
import { useAllTreasuryCentralPostnetMP } from "@/hooks/treasury/central/postnet/mp";
import { moneyColumn } from "@/ui/grid/money";
import FiltersTreasuryCentralComponent from "../../flows/tables/filters";
import GridComponent from "../../grid";

export default function TableTreasuryCentralPostnetMP({
  user,
  title,
}: {
  user: User;
  ...
  const { movements, setMovements, mutate } = useAllTreasuryCentralPostnetMP(
    user.BranchId,
    date
  );
    ...
    <div className="bg-default-300 grow overflow-x-hidden w-full p-5">
      ...
      {movements && (
        <GridComponent
          ...
          mutate={mutate}
          BranchId={user.BranchId}
          fetcher={fetchBulkCreatePostnetMP}
        />
      )}
    </div>
  );
