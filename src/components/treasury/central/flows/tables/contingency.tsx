...
import { fetchBulkCreateFlowContingency } from "@/api/endpoints/treasury/central/flow/contingency";
import { useGetConceptsByLevel } from "@/hooks/concept";
import { useAllFlowContingency } from "@/hooks/treasury/central/flow/contingency";
import { selectColumn } from "@/ui/grid/select";
import GridComponent from "../../grid";

export default function TableFlowContingency({
  user,
  title,
}: {
  user: User;
    ...
    <div className="bg-default-300 grow overflow-x-hidden w-full p-5">
      ...
      {concepts.length > 0 && movements && (
        <GridComponent
          ...
          mutate={mutate}
          BranchId={user.BranchId}
          fetcher={fetchBulkCreateFlowContingency}
        />
      )}
    </div>
  );
