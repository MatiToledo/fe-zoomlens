...
import { useAllFlowFixedCost } from "@/hooks/treasury/central/flow/fixedCost";
import GridComponent from "../../grid";
import { fetchBulkCreateFlowFixedCost } from "@/api/endpoints/treasury/central/flow/fixedCost";
import { selectColumn } from "@/ui/grid/select";

export default function TableFlowFixedCost({
  user,
  title,
}: {
  user: User;
  ...
  const { movements, setMovements, mutate } = useAllFlowFixedCost(
    user.BranchId,
    date
  );
    ...
    <div className="bg-default-300 grow overflow-x-hidden w-full p-5">
      ...
      {concepts.length > 0 && movements && (
        <GridComponent
          ...
          mutate={mutate}
          BranchId={user.BranchId}
          fetcher={fetchBulkCreateFlowFixedCost}
        />
      )}
    </div>
  );
